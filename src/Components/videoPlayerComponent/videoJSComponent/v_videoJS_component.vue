  ...
  <div class="videoJSComponent">
    <transition name="fade">
      <div v-show="overlayActive" class="overlay">
        <h3 v-bind:style="{ fontSize: fontSize + 'px' }" class="title">
          {{ title }}
        </h3>
        <div v-show="specsActive" class="specs">
          <h3 class="resolution">
            {{ translation.width + " x " + translation.height }}
          </h3>
  ...
  methods: {
    ...
    getSubtitles: function () {
      return new Promise((resolve, reject) => {
        ...
        if (this.assUrl != null) {
          if (
            this.assUrl.includes("/translations/ass/") ||
            this.assUrl.includes("/episodeTranslations/")
          ) {
            ...
            let options = {
              ...
              workerUrl:
                "https://cdn.jsdelivr.net/gh/kolyazoloto/tizen_vue_webpack_template/src/Components/videoPlayerComponent/videoJSComponent/assOctopus/subtitles-octopus-worker.js", // Link to WebAssembly-based file "libassjs-worker.js"
              legacyWorkerUrl:
                "https://cdn.jsdelivr.net/gh/kolyazoloto/tizen_vue_webpack_template/src/Components/videoPlayerComponent/videoJSComponent/assOctopus/subtitles-octopus-worker-legacy.js", // Link to non-WebAssembly worker
              blendRender: true,
            };
    ...
    overlayActive: function (val) {
      if (val === true) {
        if (this.fontSize == null) {
          ...
          this.$nextTick(() => {
            let width = this.$el
              .querySelector(".overlay .title")
              .getBoundingClientRect().width;
            let widthScroll =
              this.$el.querySelector(".overlay .title").scrollWidth;
