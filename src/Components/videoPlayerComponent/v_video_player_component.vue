      ...
      <transition name="fadeLR">
        <template v-if="playerStatusMenuActive">
          <div
            tabindex="-1"
            v-focus
            @focus="mainmenuSubGetFocus"
            v-if="dataLoadingComplete"
            class="mainmenuSub"
          >
            <div class="infoSection">
              <h3 :style="{ fontSize: fontSize + 'px' }" class="title">
                {{ animeData.russian }}
              </h3>
              <div class="statistics">
                ...
                <h3 v-if="hasVideo" class="episodes">
                  {{ animeData.episodes }} EP
                </h3>
                <h3 class="kind">{{ kind }}</h3>
              </div>
              <h3
                v-if="animeData.next_episode_at != null"
                class="nextEpisodeAt"
              >
                Новый эпизод {{ nextEpisodeAt }}
              </h3>
            </div>
                ...
                <div class="li">
                  ...
                  >
                    <transition name="fadeRate" mode="out-in">
                      <div
                        class="luRateNoActiveContainer"
                        v-if="userAnimescore == 0 && !luRateActive"
                        key="one"
                      >
                      ...
                      >
                        ...
                        ></StarRating>
                        <h3
                          v-if="userAnimescore !== undefined"
                          class="userAnimescore"
                        >
                          {{ userAnimescore.toFixed(1) }}
                        </h3>
                      </div>
                    </transition>
                  </div>
                  ...
                  <div
                    v-if="similarAnimeData.length > 0"
                    @keydown.down.prevent="leftMenuPressDown"
                    @keydown.up.prevent="leftMenuPressUp"
                  ...
                  <div
                    v-if="franchiseAnimeData.nodes.length > 0"
                    @keydown.down.prevent="leftMenuPressDown"
                    @keydown.up.prevent="leftMenuPressUp"
              ...
              <div class="rightMenuSection">
                ...
                <transition name="fadeRL">
                  <div v-if="menuActive === 2" class="translationsMenu">
                    ...
                    >
                      ...
                      <div class="specs">
                        <h3 class="typeKind">
                          {{ item.typeKind.toUpperCase() }}
                        </h3>
                        <h3 class="typeLang">
                          {{ item.typeLang.toUpperCase() }}
                        </h3>
                        <h3 class="qualityType">
                          {{ item.qualityType.toUpperCase() }}
                        </h3>
                        <h3 class="resolution">
                ...
                <transition name="fadeRL">
                  <div v-if="menuActive === 3" class="similarMenu">
                    <div
                      class="similarMenuCard"
                      v-for="(item, index) in similarAnimeData"
                      :key="index"
                      tabindex="-1"
                      :index="index"
                      @keydown.down.prevent="episodesMenuCardPressDown"
                    ...
                    >
                      <img
                        :src="'https://shikimori.me' + item.image.original"
                      />
                      <div class="info">
                        <h3 class="name">
                ...
                <transition name="fadeRL">
                  ...
                  >
                    <div
                      class="franchiseMenuCard"
                      v-for="(item, index) in franchiseAnimeData.nodes"
                      :key="index"
                      tabindex="-1"
                      :index="index"
                      @keydown.down.prevent="episodesMenuCardPressDown"
...
export default {
    ...
    playerStatusMenuActive: function (val) {
      if (val === true) {
        if (this.fontSize == null) {
          this.$nextTick(() => {
            let width = this.$el
              .querySelector(".mainmenuSub .title")
              .getBoundingClientRect().width;
            let widthScroll = this.$el.querySelector(
              ".mainmenuSub .title"
            ).scrollWidth;
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        this.$store.dispatch("refreshAccesTokenIfNeeded").then(() => {
          fetch(`https://shikimori.me/api/animes/${id}`, {
            ...
            headers: {
              Authorization:
                "Bearer " + this.$store.state.memory.shiki.access_token,
            },
          })
            .then((r) => {
              if (!r.ok) throw r;
    ...
    getSimilarAnimeData: function (id, repeatReq) {
      return new Promise((resolve, reject) => {
        this.$store.dispatch("refreshAccesTokenIfNeeded").then(() => {
          fetch(`https://shikimori.me/api/animes/${id}/similar`, {
            method: "GET",
            headers: {
              Authorization:
                "Bearer " + this.$store.state.memory.shiki.access_token,
    ...
    getFranchiseAnimeData: function (id, repeatReq) {
      return new Promise((resolve, reject) => {
        this.$store.dispatch("refreshAccesTokenIfNeeded").then(() => {
          fetch(`https://shikimori.me/api/animes/${id}/franchise`, {
            method: "GET",
            headers: {
              Authorization:
                "Bearer " + this.$store.state.memory.shiki.access_token,
            ...
            .catch((err) => {
              ...
              if (code == 429) {
                setTimeout(() => {
                  if (repeatReq < 2)
                    this.getFranchiseAnimeData(id, ++repeatReq);
                }, 1000);
              } else {
                err.json().then((json) => {
                  this.$store.commit("changeGlobalNatification", {
    ...
    getEpisodes: function (id) {
      return new Promise((resolve, reject) => {
        fetch(
          `https://smotret-anime.online/api/series/?myAnimeListId=${id}&access_token=${this.$store.state.memory.smotretAnimeRU.access_token}&fields=episodes`,
          {
            method: "GET",
          }
        )
    ...
    getTranslations: function (id) {
      return new Promise((resolve, reject) => {
        fetch(
          `https://smotret-anime.online/api/episodes/${id}?fields=translations&access_token=${this.$store.state.memory.smotretAnimeRU.access_token}`,
          {
            method: "GET",
          }
        )
          ...
          .then((json) => {
            ...
            if (translationsData === undefined)
              throw { type: "noTranslations" };
            //console.log(translationsData)
            //Загружаю только русскую озвучку и субтитры
            let resolveData = translationsData.filter(
              (item) => item.height != 0 && item.typeLang === "ru"
            );
            if (resolveData.length == 0) {
              ...
              this.$store.commit("changeGlobalNatification", {
                type: "warn",
                message:
                  "Уданной серии нет русской озвучки. Загружаю все доступные озвучки.",
                code: "SmotreAnimeRU",
              });
            } else {
          ...
          .catch((err) => {
            ...
            if (err.type === "noTranslations") {
              this.$store.commit("changeGlobalNatification", {
                type: "error",
                message:
                  "Что-то поломалось. Не смог получить список переводов.",
                code: "SmotreAnimeRU",
              });
            }
    ...
    getVideo: function (id) {
      return new Promise((resolve, reject) => {
        fetch(
          `https://smotret-anime.online/api/translations/embed/${id}?access_token=${this.$store.state.memory.smotretAnimeRU.access_token}`,
          {
            method: "GET",
          }
        )
    ...
    findCurrentEpisode: function (episodesData, currentEpisodeShiki) {
      ...
      if (currentEpisodeShiki == this.animeData.episodes) {
        this.shikiUserRateUpdate(
          `user_rate[episodes]=0&user_rate[status]=rewatching`
        ).then((data) => {
      ...
      if (animeInMemory === undefined) return translationsData[0];
        ...
        if (translationsInMemory === undefined) return translationsData[0];
          ...
          let returnvalue = translationsData.find((item) => {
            return (
              item.priority == translationsInMemory.priority &&
              item.authorsList.includes(translationsInMemory.authorsList[0])
            );
          });
    ...
    loadData: function (id) {
      return new Promise((resolve, reject) => {
        ...
        this.getAnimeData(id, 0).then((data) => {
          ...
          if (data.user_rate === null) {
            ...
            setTimeout(() => {
              this.shikiUserRateCreate(0).then((user_rate_data) => {
                ...
                console.log(
                  "обновил добавил ание к себе в запланированные",
                  this.userCurrentEpisodeShiki
                );
          ...
          this.getEpisodes(data.myanimelist_id).then(
            (episodesData) => {
              // console.log(episodesData)
              //Выбрать нужный мне эпизод
              this.choosenEpisode = this.findCurrentEpisode(
                episodesData,
                userCurrentEpisodeShiki
              );
              //   console.log(this.choosenEpisode)
              // Загрузить со SmotretAnime список озвучки для эпизода
              this.getTranslations(this.choosenEpisode.id).then(
                (translationsData) => {
                  //   console.log(translationsData)
                  //Выбрать озвучку
                  this.choosenTranslation =
                    this.findTranslations(translationsData);
    ...
    shikiUserRateUpdate: function (params, repeatReq) {
      return new Promise((resolve, reject) => {
        this.$store.dispatch("refreshAccesTokenIfNeeded").then(() => {
          fetch(
            `https://shikimori.me/api/v2/user_rates/${this.animeData.user_rate.id}?${params}`,
            {
              method: "PATCH",
              headers: {
                Authorization:
            ...
            .catch((err) => {
              ...
              if (code == 429) {
                setTimeout(() => {
                  if (repeatReq < 2)
                    this.shikiUserRateUpdate(params, ++repeatReq);
                }, 1000);
              } else {
                err.json().then((json) => {
                  this.$store.commit("changeGlobalNatification", {
    ...
    shikiUserRateCreate: function (repeatReq) {
      return new Promise((resolve, reject) => {
        this.$store.dispatch("refreshAccesTokenIfNeeded").then(() => {
          fetch(
            `https://shikimori.me/api/v2/user_rates?user_rate[target_id]=${this.animeData.id}&user_rate[user_id]=${this.$store.state.shikimori.user_id}&user_rate[target_type]=Anime`,
            {
              method: "POST",
              headers: {
                Authorization:
    ...
    shikiUserRateIncrement: function (repeatReq) {
      return new Promise((resolve, reject) => {
        this.$store.dispatch("refreshAccesTokenIfNeeded").then(() => {
          fetch(
            `https://shikimori.me/api/v2/user_rates/${this.animeData.user_rate.id}/increment`,
            {
              method: "POST",
              headers: {
                Authorization:
    ...
    leftMenuRatePressEnter: function () {
      this.shikiUserRateUpdate(`user_rate[score]=${this.userAnimescore}`).then(
        (data) => {
