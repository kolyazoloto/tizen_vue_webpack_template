<template>
  <transition name="fade">
    <div v-if="animeInfoActiveStatus" class="animeInfoComponent">
      <div class="screen">
        <img
          v-if="activeAnimeData.fullData.screenshots.length > 0"
          :src="
            'https://shikimori.me/' +
            activeAnimeData.fullData.screenshots[
              Math.floor(
                Math.random() * activeAnimeData.fullData.screenshots.length
              )
            ].original
          "
        />
...
export default {
  ...
  computed: {
    ...
    year: function () {
      let temp = new Date(
        this.$store.state.activeAnimeData.fullData.aired_on
      ).getUTCFullYear();
      return temp;
    },
    rating: function () {
