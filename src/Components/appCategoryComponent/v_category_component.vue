<template>
  <div class="categoryElem" tabindex="-1" @focus="getFocus">
    ...
    <div class="category">
      ...
      >
        <animeCard :imgURL="item.image.original"></animeCard>
      </div>
...
export default {
  ...
  methods: {
    ...
    pressEnter: function (event) {
      ...
      let catIndex =
        event.target.parentElement.parentElement.getAttribute("categoryindex");
    ...
    pressRight: function (event) {
      ...
      if (
        nextElem != null &&
        !nextElem.classList.contains("returnToCategoryStart")
      ) {
    ...
    pressLeft: function (event) {
      ...
      if (prevElem != null) {
        ...
        let elid = event.target.getAttribute("elid");
        let catIndex =
          elem.parentElement.parentElement.getAttribute("categoryindex");
        this.$store.commit("updateLastActiveAnimeCard", [elid, catIndex]);
    ...
    pressDown: function (event) {
      ...
      if (nextElem != null) {
        ...
        let parentElementCoords =
          nextElem.parentElement.getBoundingClientRect();
        let nextElemCoords = nextElem.getBoundingClientRect();
    ...
    getFocus: function (event) {
      ...
      if (this.activeAnimeCard == undefined) {
        elem
          .getElementsByClassName("animeCardComp")[0]
