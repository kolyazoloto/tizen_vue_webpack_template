  ...
  actions: {
    ...
    listDirectory({ state, dispatch }) {
      return new Promise((resolve, reject) => {
        ...
        //console.log("LISTDIR")
        filesystem.resolve(
          "wgt-private/smotretanimeRU.txt",
          onsuccess,
          onerror
        );
      });
    ...
    refreshAccesTokenIfNeeded({ dispatch, state }) {
      return new Promise((resolve, reject) => {
        if (
          Math.floor(Date.now() / 1000) - state.memory.shiki.created_at <
          86000
        ) {
        ...
        fetch(
          ...
          .catch((err) => {
            ...
            if (code == 429) {
              setTimeout(() => {
                if (repeatReq < 2)
                  dispatch("getOngoingPersonalData", ++repeatReq);
              }, 1000);
            } else {
              err.json().then((json) => {
                commit("changeGlobalNatification", {
    ...
    getAllAnimeCategoryData({ dispatch, state }) {
      ...
      for (let i = 0; i < categories.length; i++) {
        setTimeout(dispatch, 350 * i, "getOneAnimeCategoryData", {
            ...
            if (code == 401) {
              err.json().then((json) => {
                ...
                dispatch("shikiRefreshAccessToken").then(() => {
                  setTimeout(() => {
                    if (repeatReq < 2)
                      dispatch("getOneAnimeCategoryData", {
                        category: category,
                        repeatReq: ++repeatReq,
                      });
                  }, 1000);
    ...
    smotretAnimeRuAutorisation(
      { commit, dispatch, state },
      { email, password }
    ) {
      fetch(
        `https://smotret-anime.online/api/login?app=kolyazoloto&email=${email}&password=${password}`,
        {
          method: "GET",
        }
      )
